...
SARONITE_DIR=saronite
SARONITED_EXEC=saronited
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/saronite-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    SARONITED_EXEC=saronited.exe
fi
...
get_tag
popd
echo "var GUI_SARONITE_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../saronite-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy saronited to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$SARONITE_DIR/bin/$SARONITED_EXEC $BIN_PATH
fi

make deploy
popd

cp saronite_default_settings.ini build/$BIN_PATH/saronite.ini
