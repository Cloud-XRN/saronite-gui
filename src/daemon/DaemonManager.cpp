...
{
    // prepare command line arguments and pass to saronited
    ...
    qDebug() << "starting saronited " + m_saronited;
    ...
    // Start saronited
    bool started = m_daemon->startDetached(m_saronited, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM saronited.exe");
#else
                QProcess::execute("pkill saronited");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./saronited status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd, NetworkType::Type nettype, QString &message) const
{
    ...
    p.start(m_saronited, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to saronited
#ifdef Q_OS_WIN
    m_saronited = QApplication::applicationDirPath() + "/saronited.exe";
#elif defined(Q_OS_UNIX)
    m_saronited = QApplication::applicationDirPath() + "/saronited";
#endif

    if (m_saronited.length() == 0) {
